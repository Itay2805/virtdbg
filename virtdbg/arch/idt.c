 ...
 */
 ...
 static void common_exception_stub() {
    asm(
        ".cfi_signal_frame\n"
        ".cfi_def_cfa %rsp, 0\n"
        ".cfi_offset %rip, 16\n"
...
static void kernel_exception_handler(exception_context_t* ctx) {
...
static list_t m_exception_handlers = { 0 };

void hook_exception_handler(exception_handler_t* handler) {
    list_push(&m_exception_handlers, &handler->link);
}
...
void common_exception_handler(exception_context_t* ctx) {
    ...
    // run all of the exception hooks
    for (list_entry_t* entry = m_exception_handlers.next; entry != &m_exception_handlers; entry = entry->next) {
        exception_handler_t* item = CR(entry, exception_handler_t, link);
        CHECK_AND_RETHROW(item->handle(ctx, &handled));
    }

cleanup:
    if (IS_ERROR(err) || !handled) {
...
static list_t m_interrupt_handlers[0xff - 0x20 + 1] = { 0 };

void hook_interrupt_handler(interrupt_handler_t* handler, int vector) {
    DEBUG_ASSERT(0x20 <= vector && vector <= 0xff);
    list_push(&m_interrupt_handlers[vector], &handler->link);
}
...
#define INTERRUPT_HANDLER(num) \
    ...
    static void interrupt_handle_##num(void* frame) { \
        err_t err = NO_ERROR; \
        bool handled; \
        list_t* lst = &m_interrupt_handlers[num - 0x20]; \
        for (list_entry_t* entry = lst->next; entry != lst; entry = entry->next) { \
            interrupt_handler_t* item = CR(entry, interrupt_handler_t, link); \
            CHECK_AND_RETHROW(item->handle(num, item->ctx, &handled)); \
        } \
    cleanup: \
        if (IS_ERROR(err)) { \
            ERROR("Interrupt handler failed!"); \
            while(1) cpu_sleep(); \
        } \
        if (!handled) { \
            WARN("Unhandled interrupt %d", num); \
        } \
    }
 ...
 */
static idt_entry_t m_idt_entries[0xFF + 1];
...
static void set_idt_entry(uint8_t i, void(*handler)()) {
    ...
    m_idt_entries[i].selector = GDT_CODE;
...
void init_idt() {
    init_list(&m_exception_handlers);
    for (int i = 0; i < ARRAY_LEN(m_interrupt_handlers); i++) {
        init_list(&m_interrupt_handlers[i]);
    }
